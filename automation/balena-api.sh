...
print_appID_from_appName() {
	...
	local _appID=""
	local _json=""
	_json=$(${CURL} -XGET "https://api.${_apiEnv}/${TRANSLATION}/application?\$filter=(app_name%20eq%20'${_appName}')%20and%20(is_public%20eq%20true)" -H "Content-Type: application/json" -H "Authorization: Bearer ${_token}")
	pp_json "${_json}"
